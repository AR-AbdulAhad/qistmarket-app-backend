...
async function importCleanupCheck(row, superAdminId, officerId) {
  const res = mockRes();
  await commitLegacyImport({ body: { rows: [row], officer_id: officerId }, user: { id: superAdminId } }, res);
  ...
  console.log('assigned_to (VO):', order.assigned_to?.full_name, '- should be the officer, not the admin');
  console.log('delivery_officer:', order.delivery_officer?.full_name, '- should be the officer, not the admin');
  console.log('recovery_officer:', order.recovery_officer?.full_name || '(none)');
  console.log('verification.verification_officer_id === officerId:', order.verification.verification_officer_id === officerId);
  console.log('delivery.delivery_agent_id === officerId:', order.delivery.delivery_agent_id === officerId);
  console.log('statusHistories count:', order.statusHistories.length, '(expect 5)');
  console.log('progression:', order.statusHistories.map(h => `${h.old_status || 'null'}->${h.new_status} (${h.role_name})`).join(', '));
  console.log('"approved" stage attributed to admin, rest to officer:', order.statusHistories.every(h => h.new_status === 'approved' ? h.user_id === superAdminId : h.user_id === officerId));
